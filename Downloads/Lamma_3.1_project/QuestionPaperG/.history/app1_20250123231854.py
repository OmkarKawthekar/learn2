...
def generate_questions():
    """
    Generate questions based on user input and store them in the database.
    """
    try:
        data = request.json
        ...
        if not syllabus or not units:
            return jsonify({"error": "Syllabus and units are required."}), 400

        # Clear existing questions in the database
        clear_questions()

        # Store new questions in the database
        for unit, questions in units.items():
            store_questions({unit: questions})
